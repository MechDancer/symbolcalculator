...
interface Expression {
    fun d(v: Variable): Expression = TODO()
    fun substitute(v: Variable, c: Constant): Expression = TODO()
}

/** 运算成分（和式（递归成分）或变量（终止成分）） */
...
/** 变量 */
inline class Variable(val name: String) : Expression, Member

// -----------------------

/** 因子，作为积式成分而不可合并的对象 */
sealed class Factor : Expression

/** 幂因子 */
class Power private constructor(val member: Member, val exponent: Constant) : Factor()

/** 指数因子 */
class Exponential private constructor(val base: Constant, val member: Member) : Factor()

/** 自然对数因子 */
class Ln private constructor(val member: Member) : Factor()
...
/** 和式 */
class Sum private constructor(val products: Map<Factor, Constant>, val tail: Constant)
    : Calculation(), Member

/** 积式 */
class Product private constructor(val factors: Set<Factor>)
    : Calculation()
